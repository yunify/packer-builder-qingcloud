---
description: >
  The qingcloud builder is used to create endless Packer plugins using
  a consistent plugin structure.
page_title: qingcloud - Builders
nav_title: qingcloud
---
...
- `mock` (string) - The name of the mock to use for the qingcloud API.
...
- `mock_api_url` (string) - The qingcloud API endpoint to connect to.
...
```hcl
 source "qingcloud" "example" {
   mock = "bird"
 }

 build {
   sources = ["source.qingcloud.example"]
 }
```
