...
- A builder ([builder/qingcloud](builder/qingcloud))
- Docs ([docs](docs))
...
Then source the built binary to the plugin path with `cp packer-plugin-qingcloud ~/.packer.d/plugins/packer-plugin-qingcloud`
...
This qingcloud configures a [manually triggered plugin test action](/.github/workflows/test-plugin-example.yml).
...
To include plugin docs on Packer.io a global pre-hook has been added to the main qingcloud .goreleaser.yml file, that if uncommented will generate and include a docs.zip file as part of the plugin release.
...
```json
{
   "title": "qingcloud",
   "path": "qingcloud",
   "repo": "hashicorp/packer-plugin-qingcloud",
   "version": "latest",
   "sourceBranch": "main"
 }
```
...
This qingcloud template is compatible with Packer >= v1.7.0
